...
int audit_init(void) {
    ...
    if (audit_check_lock_output()) {
        mwarn("Audit rule '-a never,task' is blocking the audit output. Whodata not started.");
        // Send alert
        char msg_alert[512 + 1];
        snprintf(msg_alert, 512, "ossec: Audit: Rule is blocking the audit output: Whodata not started");
